...
def fetch_bodies_for_many_points(server, uuid, seg_instance, point_df, mutations=None, mapping=None, batch_size=10_000, threads=0, processes=0):
    ...
    """
    from .labelmap import fetch_labels_batched, fetch_mappings, fetch_mutations, compute_affected_bodies
    assert not threads or not processes, "Choose either threads or processes (not both)"
    node_seg = (server, uuid, seg_instance)

    if 'sv' not in point_df.columns:
        # Fetch supervoxel under all coordinates
        point_df['sv'] = fetch_labels_batched(*node_seg, point_df[['z', 'y', 'x']].values, supervoxels=True,
        ...
        if mutations is None:
            ...
            mutations = fetch_mutations(*node_seg, dag_filter=None)
        ...
        if len(out_of_date) > 0:
            point_df.loc[out_of_date, 'sv'] = fetch_labels_batched(*node_seg, point_df.loc[out_of_date, [*'zyx']].values, supervoxels=True,
